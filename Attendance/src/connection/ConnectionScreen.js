...
export default class ConnectionScreen extends React.Component {
  ...
  async connect() {
    try {
      ...
      alert(connection)
      // console.log(this.props.device.address)
      console.log((this.props.device).stringify());
      if (!connection) {
        this.addData({
  ...
  async onReceivedData(event) {
    ...
    console.log(Number(event.data)===5)
  ...
  async sendData() {
    try {
      ...
      // alert(message);
      alert(this.state.data[1].data);
      // alert(this.state.data[1].type);
      ...
      // let name = await this.props.device.write(data);
      // alert(this.props.device.address);
      //  alert(name)
      this.addData({
        timestamp: new Date(),
  ...
  render() {
    ...
    return (
      <View>
        <Button title="Back Arrow" transparent onPress={this.props.onBack}>
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              ...
              <View>
                <Text>{item.timestamp.toLocaleDateString()}</Text>
                <Text>
                  {item.type === 'sent' ? ' Data Sent ' : ' Not Sent '}
                </Text>
